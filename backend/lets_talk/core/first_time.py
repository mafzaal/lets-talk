...
from lets_talk.shared.config import (
    ...
    FIRST_TIME_JOB_ID, FIRST_TIME_JOB_DELAY_SECONDS, FIRST_TIME_JOB_FORCE_RECREATE, sanitize_database_url_for_logging
)
...
def is_database_empty() -> bool:
    ...
    try:
        ...
        # Also check if health check failed (database doesn't exist)
        has_errors = health_info.get("error") is not None
        
        return (no_revision and no_migrations) or has_errors
    except Exception as e:
        logger.warning(f"Could not determine database state: {e}")
        # If we can't determine, treat as empty (likely first time)
        return True
...
def create_first_time_marker() -> bool:
    ...
    try:
        ...
        with open(marker_path, 'w') as f:
            ...
            f.write(f"Database URL: {sanitize_database_url_for_logging(DATABASE_URL)}\n")
...
def is_first_time_execution() -> bool:
    ...
    try:
        ...
        # If majority of indicators suggest first time, OR if database is empty (strong indicator), consider it first time
        first_time_count = sum(indicators.values())
        total_indicators = len(indicators)
        
        # Strong indicators: database empty or no output files
        strong_first_time_indicators = indicators.get("database_empty", False) or indicators.get("no_output_files", False)
        
        # Consider it first time if:
        # 1. Majority of indicators suggest first time, OR
        # 2. Database is empty (even if other indicators say otherwise), OR  
        # 3. At least half the indicators and no marker file exists
        is_first_time = (
            first_time_count >= (total_indicators // 2 + 1) or  # Majority
            indicators.get("database_empty", False) or           # Database empty is strong indicator
            (first_time_count >= (total_indicators // 2) and not has_first_time_marker())  # Half + no marker
        )
